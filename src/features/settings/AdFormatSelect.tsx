...
/**
 * Multi-select dropdown for the offer ad formats: a compact trigger summarising the
 * selection, opening a checkbox list. Stays open while toggling; closes on outside tap.
 */
export function AdFormatSelect() {
  ...
  const summary =
    selected.length === 0
      ? 'None'
      : selected.length === 1
        ? (AD_FORMATS.find((format) => format.value === selected[0])?.label ?? '1 selected')
      ...
      {open && (
        <div className="absolute right-0 top-full z-10 mt-1.5 w-44 rounded-xl border border-line bg-screen p-1 shadow-[0_12px_32px_#00000033]">
          {AD_FORMATS.map((format) => {
            const active = selected.includes(format.value)
            return (
              <button
                key={format.value}
                type="button"
