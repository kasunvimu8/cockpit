...
export function Toast() {
  ...
  return (
    <div
      className={`pointer-events-none absolute left-1/2 top-3 z-7 whitespace-nowrap rounded-full border border-btn-border bg-surface px-4 py-[7px] text-[11.5px] font-semibold text-teal shadow-[0_4px_16px_#00000014] backdrop-blur-sm transition-all duration-[400ms] ${visible ? '-translate-x-1/2 translate-y-0 opacity-100' : '-translate-x-1/2 -translate-y-2.5 opacity-0'}`}
    >
      {message}
    </div>
  )
